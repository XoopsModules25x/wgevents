...
<div id='imghandler' class='col-xs-12 col-sm-12'>
    <ul class="nav nav-tabs" id="myTab" role="tablist">
        <li class="nav-item" role="presentation">
            <button class="nav-link active" id="home-tab" data-toggle="tab" data-target="#ie_current" type="button" role="tab" aria-controls="ie_current" aria-selected="true"><{$smarty.const._MA_WGEVENTS_IMG_EDITOR_CURRENT}></button>
        </li>
        <li class="nav-item" role="presentation">
            <button class="nav-link" id="profile-tab" data-toggle="tab" data-target="#ie_existing" type="button" role="tab" aria-controls="ie_existing" aria-selected="false"><{$smarty.const._MA_WGEVENTS_IMG_EDITOR_USE_EXISTING}></button>
        </li>
        <li class="nav-item" role="presentation">
            <button class="nav-link" id="contact-tab" data-toggle="tab" data-target="#ie_grid" type="button" role="tab" aria-controls="ie_grid" aria-selected="false"><{$smarty.const._MA_WGEVENTS_IMG_EDITOR_GRID}></button>
        </li>
        <li class="nav-item" role="presentation">
            <button class="nav-link" id="contact-tab" data-toggle="tab" data-target="#ie_crop" type="button" role="tab" aria-controls="ie_crop" aria-selected="false"><{$smarty.const._MA_WGEVENTS_IMG_EDITOR_CROP}></button>
        </li>
        <li class="nav-item" role="presentation">
            <button class="nav-link" id="contact-tab" data-toggle="tab" data-target="#ie_upload" type="button" role="tab" aria-controls="ie_upload" aria-selected="false"><{$smarty.const._MA_WGEVENTS_FORM_UPLOAD_IMG}></button>
        </li>
    </ul>
    <div class="tab-content" id="myTabContent">
        <!-- *************** Basic Tab ***************-->
        <div class="tab-pane fade show active center" id="ie_current" role="tabpanel" aria-labelledby="home-tab">
            ...
            <input type='button' class='btn btn-success' value='<{$smarty.const._CANCEL}>' onclick='history.go(-1);return true;'>
        </div>
        <!-- *************** Tab for select image of albums ***************-->
        <div class="tab-pane fade" id="ie_existing" role="tabpanel" aria-labelledby="profile-tab">
            <div class="row">
                <div class='col-xs-12 col-sm-6'>
                    <{foreach item=image from=$images name=fe_image}>
                        <{if $image.group|default:false}><h4 class='modal-title'><{$image.group}></h4><{/if}>
                        <span class='imageeditor-selimages col-xs-12 col-sm-3'>
                            <input id='<{$image.name}>_image' class='imgSelect1 imageeditor-img <{if $image.selected|default:false}>imageeditor-modal-selected<{/if}>' type='image' src='<{$image.src}>' alt='<{$image.title}>' style='max-height:200px;max-width:200px;padding:3px;' value='<{$image.name}>'>
                        </span>
                    <{/foreach}>
                </div>
                <div class='col-xs-12 col-sm-6'>
            ...
            <div class='col-xs-12 col-sm-12 center'>
                <form class='form-horizontal' name='form' id='form_selectimage' action='image_editor.php' method='post' enctype='multipart/form-data'>
                    ...
                    <input type="submit" class="btn btn-primary disabled" name="btnApplySelected" id="btnApplySelected" value="<{$smarty.const._MA_WGEVENTS_IMG_EDITOR_APPLY}>">
                    <input type='button' class='btn btn-success' value='<{$smarty.const._CANCEL}>' onclick='history.go(-1);return true;'>
                </form>
            </div>
        </div>
        <!-- *************** Tab for image grid ***************-->
        <div class="tab-pane fade" id="ie_grid" role="tabpanel" aria-labelledby="contact-tab">
            <div class="row">
            ...
            <div class="row">
                <div class='col-xs-12 col-sm-12 center'>
                    ...
                    <form class='form-horizontal' name='form' id='form_imagegrid' action='image_editor.php' method='post' enctype='multipart/form-data'>
                        ...
                        <input type="submit" class="btn btn-primary disabled" name="btnApplyGrid" id="btnApplyGrid" value="<{$smarty.const._MA_WGEVENTS_IMG_EDITOR_APPLY}>">
                        <input type='button' class='btn btn-success' value='<{$smarty.const._CANCEL}>' onclick='history.go(-1);return true;'>
                    </form>
                </div>
            </div>
        ...
        <!-- ***************Tab for crop image ***************-->
        <div class="tab-pane fade" id="ie_crop" role="tabpanel" aria-labelledby="contact-tab">
            ...
            <!-- Content -->
            <div class="container-crop center">
                <div class="row">
                    <div class="img-container col-12 center">
                        <img id='cropImg' class="img-responsive center" src="<{$imgCurrent.src|default:''}>" alt="<{$imgCurrent.img_name|default:''}>'">
                    </div>
                </div>
            </div>
            <div class="row" id="actions">
                <div class="col-md-12 docs-toggles center">
                    <!-- <h3>Toggles:</h3> -->
                ...
                <div class="col-md-12 docs-buttons center">
                    ...
                    <div class="btn-group-horizontal btn-group-crop col-xs-12 col-sm-12">
                        ...
                        <a class="btn btn-primary disabled" id="btnCropApply" href="<{$wgevents_image_editor}>/image_editor.php?op=saveCrop&<{$imageOrigin|default:''}>=<{$imageId|default:0}>&target=<{$croptarget|default:''}>&start=<{$start|default:0}>&limit=<{$limit|default:0}>"> <{$smarty.const._MA_WGEVENTS_IMG_EDITOR_APPLY}></a>
                        <button type="button" class="btn btn-crop btn-success"onclick='history.go(-1);return true;'><{$smarty.const._CANCEL}></button>
                    </div>
        ...
        <div class="tab-pane fade" id="ie_upload" role="tabpanel" aria-labelledby="contact-tab">
            <{$form_uploadimage}>
        </div>
    </div>
</div>
