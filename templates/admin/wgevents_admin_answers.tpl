    ...
    <table class='table table-bordered' style="max-width:500px">
        ...
        <{foreach item=event from=$events_list}>
            <tr class='<{cycle values='odd, even'}>'>
                ...
                <td class="center ">
                    <a href="answers.php?op=list&amp;ev_id=<{$event.id}>&amp;start=<{$start}>&amp;limit=<{$limit}>" title="<{$smarty.const._DETAILS}>"><img src="<{xoModuleIcons16 view.png}>" alt="<{$smarty.const._DETAILS}> events" ></a>
                </td>
            </tr>
            <{/foreach}>
        </tbody>
    ...
    <table class='table table-bordered'>
        ...
        <tbody>
            ...
            <tr class='<{cycle values='odd, even'}>'>
                ...
                <td class='center'><{$answer.evid}></td>
                <td class='center'><{$answer.regname}> (<{$answer.regid}>)</td>
                <td class='center'><{$answer.addid}></td>
                <td class='center'><{$answer.text}></td>
                <td class='center'><{$answer.datecreated}></td>
                <td class='center'><{$answer.submitter}></td>
                <td class="center  width5">
                    <a href="answers.php?op=edit&amp;ans_id=<{$answer.id}>&amp;start=<{$start}>&amp;limit=<{$limit}>" title="<{$smarty.const._EDIT}>"><img src="<{xoModuleIcons16 edit.png}>" alt="<{$smarty.const._EDIT}> answers" ></a>
                    <a href="answers.php?op=clone&amp;ans_id_source=<{$answer.id}>" title="<{$smarty.const._CLONE}>"><img src="<{xoModuleIcons16 editcopy.png}>" alt="<{$smarty.const._CLONE}> answers" ></a>
                    <a href="answers.php?op=delete&amp;ans_id=<{$answer.id}>" title="<{$smarty.const._DELETE}>"><img src="<{xoModuleIcons16 delete.png}>" alt="<{$smarty.const._DELETE}> answers" ></a>
                </td>
            </tr>
            <{/foreach}>
        </tbody>
