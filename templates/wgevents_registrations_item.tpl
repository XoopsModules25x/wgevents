    ...
    <td>
        <{if $regdetails.permRegistrationApprove|default:''}>
            ...
            <{if $regdetails.listwait|default:0 > 0}>
                <a class='btn btn-primary right wge-btn-2' href='registrations.php?op=listwait_takeover&amp;redir=<{$redir}>&amp;id=<{$regdetails.id}>' title='<{$smarty.const._MA_WGEVENTS_REGISTRATION_LISTWAIT_TAKEOVER}>'><i class="fa fa-user-plus fa-fw"></i></a>
            <{/if}>
        <{/if}>
        <{if $regdetails.permRegistrationEdit|default:''}>
            <a class='btn btn-primary right wge-btn-2' href='registrations.php?op=edit&amp;redir=<{$redir}>&amp;id=<{$regdetails.id}>' title='<{$smarty.const._EDIT}>'><i class="fa fa-edit fa-fw"></i></i></a>
            <a class='btn btn-danger right wge-btn-2' href='registrations.php?op=delete&amp;redir=<{$redir}>&amp;evid=<{$regdetails.evid}>&amp;id=<{$regdetails.id}>' title='<{$smarty.const._DELETE}>'><i class="fa fa-trash fa-fw"></i></a>
        <{/if}>
    </td>
</tr>
