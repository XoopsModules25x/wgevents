    ...
    <{if $event.register_use|default:''}>
        <div class="row wge-spacer1">
            ...
            <{if $event.permEdit|default:false}>
                ...
                <div class="col-xs-12 col-sm-7"><{$event.register_listwait_text}></div>
            ...
            <div class="row wge-spacer1">
                ...
                <div class="col-xs-12 col-sm-5 right">
                    <{if $event.permEdit|default:false && $event.nb_registrations|default:0 > 0}>
                        <a class='btn btn-success wge-btn' href='registrations.php?op=listeventall&amp;redir=listeventall&amp;evid=<{$event.id}>' title='<{$smarty.const._MA_WGEVENTS_REGISTRATIONS_LIST}>'><{$smarty.const._MA_WGEVENTS_REGISTRATIONS_LIST}></a>
                    <{/if}>
                </div>
            </div>
            ...
            <div class="row">
                ...
                <div class="col-xs-12 col-sm-5 right">
                    <{if $event.nb_questions|default:0 > 0 || $event.id|default:0 > 0 }>
                        <a class='btn btn-success wge-btn' href='questions.php?op=list&amp;evid=<{$event.id}>' title='<{$smarty.const._MA_WGEVENTS_GOTO_QUESTIONS}>'><{$smarty.const._MA_WGEVENTS_GOTO_QUESTIONS}></a>
                    <{else}>
                        <a class='btn btn-success wge-btn' href='questions.php?op=newset&amp;evid=<{$event.id}>' title='<{$smarty.const._MA_WGEVENTS_GOTO_QUESTIONS}>'><{$smarty.const._MA_WGEVENTS_GOTO_QUESTIONS}></a>
                    <{/if}>
                </div>
            </div>
        <{/if}>
...
<div class='panel-foot'>
    <div class='col-sm-12 right'>
        <{if $showItem|default:false}>
            <a class='btn btn-success right wge-btn' href='events.php?op=list&amp;start=<{$start}>&amp;limit=<{$limit}>#evId_<{$event.id}>' title='<{$smarty.const._MA_WGEVENTS_EVENTS_LIST}>'><{$smarty.const._MA_WGEVENTS_EVENTS_LIST}></a>
        <{else}>
            <a class='btn btn-success right wge-btn' href='events.php?op=show&amp;id=<{$event.id}>&amp;start=<{$start}>&amp;limit=<{$limit}>' title='<{$smarty.const._MA_WGEVENTS_DETAILS}>'><{$smarty.const._MA_WGEVENTS_DETAILS}></a>
        <{/if}>
        <{if $event.register_use|default:'' &&  $permRegister|default:false}>
            <a class='btn btn-primary right wge-btn <{if !$event.regenabled|default:false}>disabled<{/if}>' href='registrations.php?op=listmyevent&amp;redir=listmyevent&amp;evid=<{$event.id}>' title='<{$smarty.const._MA_WGEVENTS_GOTO_REGISTRATION}>'><{$smarty.const._MA_WGEVENTS_GOTO_REGISTRATION}></a>
        <{/if}>
        <{if $event.permEdit|default:false || $permApprove|default:false}>
            <br><a class='btn btn-primary right wge-btn' href='events.php?op=edit&amp;id=<{$event.id}>&amp;start=<{$start}>&amp;limit=<{$limit}>' title='<{$smarty.const._EDIT}>'><{$smarty.const._EDIT}></a>
            <a class='btn btn-primary right wge-btn' href='events.php?op=clone&amp;id_source=<{$event.id}>' title='<{$smarty.const._CLONE}>'><{$smarty.const._CLONE}></a>
            <br><a class='btn btn-danger right wge-btn' href='events.php?op=cancel&amp;id=<{$event.id}>' title='<{$smarty.const._MA_WGEVENTS_EVENT_CANCEL}>'><{$smarty.const._MA_WGEVENTS_EVENT_CANCEL}></a>
            <a class='btn btn-danger right wge-btn' href='events.php?op=delete&amp;id=<{$event.id}>' title='<{$smarty.const._DELETE}>'><{$smarty.const._DELETE}></a>
        <{/if}>
    </div>
</div>
